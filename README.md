...
Official server-side client for image, PDF and video watermarking. Java 21+ and Maven 3.9+. MIT licensed.

## Install

Install locally from source (not yet published on Maven Central):
```sh
git clone --branch v0.5.0 https://github.com/etchv-labs/java-sdk.git
mvn -f java-sdk/pom.xml install
```
Then add to your application's `pom.xml`:
```xml
<dependency>
  <groupId>com.etchv</groupId><artifactId>etchv-sdk</artifactId><version>0.5.0</version>
</dependency>
```
...
Resume saved jobs with `getEmbedResult(requestId)` / `getDetectionResult(requestId)`.
